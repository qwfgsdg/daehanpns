...
export default function SocialRegisterPage() {
  ...
  // URL에서 SNS 데이터 파싱
  useEffect(() => {
    const providerParam = searchParams.get('provider') as 'google' | 'kakao';
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-600">{error || 'SNS 정보를 불러오는 중...'}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold text-gray-900">SNS 회원가입</h2>
          <p className="mt-2 text-gray-600">
            {provider === 'google' ? '구글' : '카카오'} 계정으로 가입 중
          </p>
          {snsData.email && (
            <p className="mt-1 text-sm text-gray-500">{snsData.email}</p>
          )}
          {snsData.name && (
            <p className="mt-1 text-sm text-gray-500">카카오 닉네임: {snsData.name}</p>
          )}
        </div>
